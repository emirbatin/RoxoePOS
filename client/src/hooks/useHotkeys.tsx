  ...
  const handleKeyDown = useCallback(
    (event: KeyboardEvent) => {
      ...
      hotkeys.forEach(({ key, callback, ctrlKey, metaKey, altKey, shiftKey }) => {
        ...
        if (matchesKey && matchesCtrl && matchesCmd && matchesAlt && matchesShift) {
          callback(event);
        }
      });
    },
...
export const HotkeysHelper: React.FC<HotkeysHelperProps> = () => {
  ...
  return (
    <>
      {isVisible ? (
        <div
          className="fixed bottom-4 right-4 bg-gray-800 text-white p-4 rounded-lg shadow-lg text-sm opacity-90 transition-transform transform animate-fade-in"
        >
          ...
          <ul className="space-y-1">
            <li><kbd className="bg-gray-700 px-2 py-0.5 rounded">{modKey}+N</kbd> Yeni Satış</li>
            <li><kbd className="bg-gray-700 px-2 py-0.5 rounded">{modKey}+P</kbd> Ödeme</li>
            <li><kbd className="bg-gray-700 px-2 py-0.5 rounded">ESC</kbd> İptal/Kapat</li>
            <li><kbd className="bg-gray-700 px-2 py-0.5 rounded">Enter</kbd> Barkod Ara</li>
            <li><kbd className="bg-gray-700 px-2 py-0.5 rounded">{modKey}+K</kbd> Ürün Ara</li>
            <li><kbd className="bg-gray-700 px-2 py-0.5 rounded">{modKey}+/</kbd> Yardım</li>
          </ul>
        </div>
      ) : (
        <button
