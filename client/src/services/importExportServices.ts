...
class ImportExportService {
      ...
      worksheet.eachRow((row, rowNumber) => {
        ...
        try {
          ...
          if (error instanceof ProcessingError) {
            ...
            throw new ProcessingError(`Satır ${rowNumber}: Beklenmeyen bir hata oluştu`);
          }
        }
      });
          ...
          try {
            const products: Product[] = results.data.map((row, index) => {
              try {
                ...
                if (error instanceof ProcessingError) {
                  throw new ProcessingError(`Satır ${index + 2}: ${error.message}`);
                }
                throw new ProcessingError(`Satır ${index + 2}: Beklenmeyen bir hata oluştu`);
              }
            });
            resolve(products);
          } catch (error) {
  ...
  private processExcelRow(row: ExcelJS.Row, headers: Map<string, number>): Product {
    try {
      const getValue = (header: string): string => {
        const colNumber = headers.get(header);
        if (!colNumber) throw new ProcessingError(`'${header}' sütunu bulunamadı`);
      ...
      if (isNaN(price) || price < 0) throw new ProcessingError('Geçersiz fiyat');
      if (![0, 1, 8, 18, 20].includes(vatRate)) throw new ProcessingError('Geçersiz KDV oranı');
      if (isNaN(stock) || stock < 0) throw new ProcessingError('Geçersiz stok miktarı');
  ...
  private processCSVRow(row: Record<string, string>, rowNumber: number): Product {
    try {
      if (!row['Ürün Adı']) throw new ProcessingError('Ürün adı boş olamaz');
      if (!row['Fiyat (KDV Hariç)']) throw new ProcessingError('Fiyat boş olamaz');
