...
class LicenseManager {
  ...
  constructor() {
    ...
    this.API_URL =
      process.env.API_LICENSE_API_URL || "http://localhost:3001/api/licenses";
    this.setupListeners();
  }
  ...
  private setupListeners() {
    ipcMain.handle("check-license", async () => {
      try {
        ...
        if (
          ...
          new Date() > new Date(licenseData.expiresAt)
        ) {
      ...
      try {
        ...
        if (
          ...
          const expiresAt = response.data.expires
            ? new Date(response.data.expires).toISOString()
            : null;
    ...
    ipcMain.handle("get-license-info", async () => {
      try {
        ...
        const isExpiring =
          daysLeft !== null ? daysLeft > 0 && daysLeft <= 7 : false;
        // Maskelenmiş lisans: ilk 4 ve son 4 karakter
        const maskedLicense =
          licenseData.licenseKey.slice(0, 4) +
          "-xxxx-" +
          licenseData.licenseKey.slice(-4);
        return {
          exists: true,
          maskedLicense,
          daysLeft,
